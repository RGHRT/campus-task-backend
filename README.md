...
项目采用 JWT 进行接口身份认证，使用 Django ORM 操作 MySQL 数据库，并通过数据库事务与行级锁避免任务被重复接取。目前已完成 Swagger 接口文档、34 项自动化测试及 Railway 线上部署。
...
- 使用 Redis 缓存任务列表查询结果，默认有效期为 60 秒。
- 缓存键包含状态、关键词、页码和分页大小，避免不同查询互相覆盖。
- 任务发布、修改、接取、完成、确认、取消或删除后，通过缓存版本号使旧列表缓存失效。
- 没有配置 Redis 或运行自动化测试时，自动回退到 Django 本地内存缓存。

### 接口限流

- Session 登录：同一 IP 每分钟最多 10 次。
- JWT 登录：同一 IP 每分钟最多 10 次。
- 用户注册：同一 IP 每小时最多 5 次。
- 发布任务：同一登录用户每分钟最多 5 次。
- 超过限制后返回 HTTP 429。

### Docker Compose

Docker Compose 会同时启动 Django、MySQL 和 Redis：
...
通过 34 项自动化测试验证用户模块、任务状态流转、权限控制、JWT 认证和分页搜索等核心功能。
